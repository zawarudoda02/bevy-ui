use bevy::app::App;
use bevy::math::UVec2;
use bevy::prelude::{apply_deferred, Commands, Entity, info, IntoSystemConfigs, NonSend, OnEnter, Plugin, Res, ResMut, Resource, Update, Visibility, warn, World};
...
fn get_world_info(mut commands: Commands,mut ticks:  ResMut<Ticks>){
    ...
    if let Some(x) = ticks.pop() {
        ...
        if let (Some(Message::LibEvent(LibEvent::Ready)), Some(Message::WorldInfo { spawn_point, world_size })) = (a, b) {
...
impl Plugin for WorldPlugin{
    fn build(&self, app: &mut App) {
        app
            .add_systems(OnEnter(UiStates::Setup),(get_world_info,apply_deferred,create_tile_map,apply_deferred).chain().in_set(UiSystemSet::WorldSetup))
...
fn update_weather( mut weather :ResMut<CurrentWeather>,mut tick: ResMut<CurrentTick>){
    match tick.peek(){
        ...
        Some(message) => {
            match message{
                Message::LibEvent(LibEvent::TimeChanged(x)) | Message::LibEvent(LibEvent::DayChanged(x))  => {

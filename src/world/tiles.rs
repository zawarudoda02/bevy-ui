...
pub fn setup_tiles(mut commands: Commands, map: Res<WorldTiles>, sprite_sheet: Res<SpriteSheet>, grid: Res<Grid>){
...
impl Plugin for TilesPlugin{
    fn build(&self, app: &mut App) {
        app.add_systems(OnEnter(UiStates::Setup),(setup_tiles,apply_deferred,setup_content).chain().in_set(UiSystemSet::TilesSetup))
...
fn setup_content(mut commands: Commands,query: Query<(Entity, &Transform),With<TileMarker>>,sprite_sheet: Res<SpriteSheet>){
        for (e,t) in &query{
            info!("setting up content for {:?}",e);
            let content = commands.spawn(ContentBundle::new(SpriteSheetBundle{
                sprite: TextureAtlasSprite::new(sprite_sheet.get_content_sprite_index(Content::None)),
                texture_atlas: sprite_sheet.atlas.clone(),
                transform: Transform{

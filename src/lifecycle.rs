...
use bevy::prelude::{error, info, IntoSystemConfigs, NextState, OnEnter, ResMut, Resource, Update, warn};
...
fn update_tick(mut ticks: ResMut<Ticks>, mut curr_tick: ResMut<CurrentTick>,mut next_state:ResMut<NextState<UiStates>>){
...
fn tick_control_flow(mut curr_tick: ResMut<CurrentTick>, mut next_state: ResMut<NextState<UiStates>>){
    //warn!("I'm in system 'tick_control flow', trying to parse the message");
    match curr_tick.peek() {
        None => { return;}
        Some(message) => {
            match message{
                Message::LibEvent(LibEvent::EndOfTick) => {info!("Tick ended! Awaiting next one")}
                Message::LibEvent(LibEvent::Terminated) => {info!("Lifecycle terminated!");next_state.set(UiStates::End);}
                _ =>{ return;}
            }
        }
    }
...
impl Plugin for LifeCyclePlugin{
    fn build(&self, app: &mut App) {
        app.init_resource::<CurrentTick>()
            .add_systems(OnEnter(UiStates::Setup),begin_lifecycle.in_set(UiSystemSet::BeginLifeCycle))
            .add_systems(Update,(update_tick).in_set(LifeCycleSets::UpdateTick))
            .add_systems(Update,(tick_control_flow).in_set(LifeCycleSets::ControlFlow))

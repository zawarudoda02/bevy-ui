...
use bevy::app::App;
use bevy::prelude::{IntoSystemConfigs, Plugin, ResMut, Resource, Update, warn};
...
impl BackPack{
  ...
  pub fn add_to_backpack(&mut self, c: Content, quantity: usize){
    ...
    self.contents.entry(stripped_content).and_modify(| x| *x+=quantity).or_insert( quantity);
  ...
  fn remove_from_backpack(&mut self, c: Content, quantity: usize){
    ...
    self.contents.entry(stripped_content).and_modify(|x| *x = x.checked_sub(quantity).unwrap_or_else(|| 0)).or_insert(0);
  }
